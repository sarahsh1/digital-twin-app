...
export default function SetupScreen() {
  ...
  // Known synchronously from how we got here -- Profile's Edit Profile link
  // passes mode=edit. Deciding the visual chrome from this (rather than
  // from the async AsyncStorage check below) avoids a flash between the
  // plain in-app look and the onboarding hero treatment.
  ...
  const submitButton = isEditing ? (
    ...
    >
      <Text className="text-background text-lg font-semibold">Save Changes</Text>
    </TouchableOpacity>
  ) : (
    <TouchableOpacity
      onPress={handleComplete}
    ...
    <ScrollView className="flex-1 px-6" contentContainerStyle={{ paddingTop: 40, paddingBottom: 40 }}>
      <View className="mb-8">
        <Text
          ...
          style={isEditing ? undefined : { letterSpacing: -0.5 }}
        >
  ...
  if (isEditing) {
    return <ScreenContainer edges={["top", "left", "right"]}>{formContent}</ScreenContainer>;
  }
